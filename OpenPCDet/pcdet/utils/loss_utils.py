...
class SigmoidFocalClassificationLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        ...
        # 这里增加了判断是否多帧的操作,如果是多帧target和weights就只取第0个
        # 移到了anchor_head_template.py
        # if target.shape[0]/input.shape[0]==4:
        #     target ,weights = multi_frame_process(input.shape[0],target,weights)

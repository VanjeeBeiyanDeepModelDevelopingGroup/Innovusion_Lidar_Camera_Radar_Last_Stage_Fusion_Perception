...
def clean_data(gt_anno, dt_anno, current_class):
    ...
    for i in range(num_gt):
        ...
        if valid_class == 1: #and not ignore:
            ...
            ignored_gt.append(-1)
    for i in range(num_dt):  #遍历dt_box
    ...
    #print('::::::::num_valid_gt:::::::::', num_valid_gt, ignored_gt, ignored_dt)
...
def rotate_nms_cc_new(dets,trackers):
    trackers_corners = center_to_corner_box2d(trackers[:, :2], trackers[:, 3:5], trackers[:, 6])
    trackers_standup = corner_to_standup_nd(trackers_corners)
    dets_corners = center_to_corner_box2d(dets[:, :2], dets[:, 3:5],dets[:, 6])
    dets_standup = corner_to_standup_nd(dets_corners)
    standup_iou = iou_jit(dets_standup, trackers_standup, eps=0.0)
    return standup_iou
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50, z_axis=1, z_center=1.0):
    ...
    for num_part in split_parts:
        ...
        #print("::::::::::::::gt_annos[]:::::::::::::", example_idx, example_idx + num_part)
        #if metric == 0:
...
def _prepare_data(gt_annos, dt_annos, current_class): 
    ...
    for i in range(len(gt_annos)):
        ...
        #print('::::::::::::gt_datas::::::::::', gt_datas.shape)
        #print('::::::::::::dt_annos::::::::::', (dt_annos[i]["score"][..., np.newaxis]).shape)
        dt_datas = np.concatenate([
            dt_annos[i]["location"], dt_annos[i]["alpha"][..., np.newaxis],
            dt_annos[i]["score"][..., np.newaxis]
        ], 1)
        #print('::::::::::::dt_datas::::::::::', dt_datas.shape)
...
def eval_class(gt_annos,
    ...
    all_thresholds = np.zeros([num_class, num_minoverlap, N_SAMPLE_PTS])
    for m, current_class in enumerate(current_classes): 
        ...
        for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
            ...
            all_thresholds[m, k, :len(thresholds)] = thresholds
            ...
            for i in range(len(thresholds)):
                ...
                if compute_aos:
                    aos[m, k, i] = np.max(aos[m, k, i:], axis=-1)
    #print('::::::::::::::recall::::::::::::::', recall)
    #print('::::::::::::::precision::::::::::::::', precision)
...
def do_eval_orig(gt_annos,
    ...
    return mAP_bev, mAP_3d, mAP_aos, mAP_bev_R40, mAP_3d_R40, mAP_aos_R40
...
def do_eval(gt_annos,
    ...
    for _ in range(1):
        ...
        mAP_3d = get_mAP(ret["precision"])
        mAP_3d_R40 = get_mAP_R40(ret["precision"])
    return mAP_3d, mAP_3d_R40, mAP_aos, mAP_aos_R40
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
    ...
    mAP_bev, mAP_3d, mAP_aos = do_eval_orig(
    ...
    return mAP_bev, mAP_3d, mAP_aos


def get_official_eval_result(gt_annos, dt_annos, current_classes, z_axis=1, z_center=1.0):
    overlap_0_7 = np.array([[0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], 
                            [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
                            [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]])
    overlap_0_5 = np.array([[0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25], 
    ...
    #print('name_to_class', name_to_class)
    ...
    for anno in dt_annos:
        if anno['alpha'].shape[0] != 0:
            ...
            break
    """
    mAP3d, mAP3d_R40, mAPaos, mAPaos_R40 = do_eval(gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, z_axis=z_axis, z_center=z_center)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            #result += print_str((f"bbox AP:{mAPbbox[j, 0, i]:.4f}, "
                                 ...
                                 #f"{mAPbbox[j, 2, i]:.4f}"))
            #print('::::::::::::::::::mAPbev::::::::::::::::::', mAPbev)
            #result += print_str((f"bev  AP:{mAPbev[j, i]:.4f}, "
                                 #f"{mAPbev[j, i]:.4f}, "
                                 #f"{mAPbev[j, i]:.4f}"
                                 #))
            result += print_str((f"3d   AP:{mAP3d[j, i]:.4f}, "
                                 #f"{mAP3d[j, i]:.4f}, "
                                 #f"{mAP3d[j, i]:.4f}"
                                 ))

            if compute_aos:
                result += print_str((f"aos  AP:{mAPaos[j, i]:.2f}, "
                                     #f"{mAPaos[j, i]:.2f}, "
                                     #f"{mAPaos[j, i]:.2f}"
                                     ))
                # if i == 0:
            ...
            #result += print_str((f"bev  AP:{mAPbev_R40[j, i]:.4f}, "
                                 #f"{mAPbev_R40[j, i]:.4f}, "
                                 #f"{mAPbev_R40[j, i]:.4f}"
                                 #))
            ##result += print_str((f"3d   AP:{mAP3d_R40[j, i]:.4f}, "
                                 #f"{mAP3d_R40[j, i]:.4f}, "
                                 #f"{mAP3d_R40[j, i]:.4f}"
                                 ##))
            ##if compute_aos:
                ##result += print_str((f"aos  AP:{mAPaos_R40[j, i]:.2f}, "
                                     #f"{mAPaos_R40[j, i]:.2f}, "
                                     #f"{mAPaos_R40[j, i]:.2f}"
                                     ##))
                ##if i == 0:
                   ##ret_dict['%s_aos/easy_R40' % class_to_name[curcls]] = mAPaos_R40[j, 0]
                   ##ret_dict['%s_aos/moderate_R40' % class_to_name[curcls]] = mAPaos_R40[j, 0]
                   ##ret_dict['%s_aos/hard_R40' % class_to_name[curcls]] = mAPaos_R40[j, 0]
            ...
            ##if i == 0:
                ...
                ##ret_dict['%s_3d/moderate_R40' % class_to_name[curcls]] = mAP3d_R40[j, 0]
                ##ret_dict['%s_3d/hard_R40' % class_to_name[curcls]] = mAP3d_R40[j, 0]
                #ret_dict['%s_bev/easy_R40' % class_to_name[curcls]] = mAPbev_R40[j, 0]
                #ret_dict['%s_bev/moderate_R40' % class_to_name[curcls]] = mAPbev_R40[j, 0]
                #ret_dict['%s_bev/hard_R40' % class_to_name[curcls]] = mAPbev_R40[j, 0]
    ...
    for anno in dt_annos:
        if anno['alpha'].shape[0] != 0:
            if anno['alpha'][0] != -10:
                compute_aos = True
            break
    mAPbev, mAP3d, mAPaos = do_coco_style_eval(

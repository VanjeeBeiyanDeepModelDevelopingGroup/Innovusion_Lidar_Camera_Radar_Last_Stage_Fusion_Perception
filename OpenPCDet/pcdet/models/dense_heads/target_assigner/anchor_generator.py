...
class AnchorGenerator(object):
    ...
    def generate_anchors(self, grid_sizes):
        ...
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
            ...
            # [x_grid, y_grid, z_grid] --> [(W/2,H/2,1), (W/2,H/2,1),(W/2,H/2,1)]
            # meshgrid可以理解为在原来的维度上进行扩展,例如:
            # x原来为(W/2，)-->(W/2，1, 1)--> (W/2,H/2,1)
            # y原来为（H/2，）--> (1，H/2，1）--> (W/2,H/2,1) 
            # z原来为 (1,) --> (1,1,1) --> (W/2,H/2,1)

            # 2.4.anchor各个维度堆叠组合，生成最终anchor(1,248,216,1,2,7）
            # 2.4.1.堆叠anchor的位置
            anchors = torch.stack((x_shifts, y_shifts, z_shifts), dim=-1)  # [x, y, z, 3] --> [W/2,H/2,1,3]
            # 2.4.2.将anchor的位置和大小进行组合，编程套路为将anchor扩展并复制为相同维度（除了最后一维），然后进行组合
            anchors = anchors[:, :, :, None, :].repeat(1, 1, 1, anchor_size.shape[0], 1) # [W/2,H/2,1,3] --> [W/2,H/2,1,1,3]
            anchor_size = anchor_size.view(1, 1, 1, -1, 3).repeat([*anchors.shape[0:3], 1, 1]) # (1,1,1,1,3)-->(W/2,H/2,1,1,3)
            anchors = torch.cat((anchors, anchor_size), dim=-1) # anchors的位置+大小 --> (W/2,H/2,1,1,6)
            # 2.4.3.将anchor的位置和大小和旋转角进行组合
            anchors = anchors[:, :, :, :, None, :].repeat(1, 1, 1, 1, num_anchor_rotation, 1) #[200, 200, 1, 1, 2, 6]
            # (200,200,1,1,2,1)
            anchor_rotation = anchor_rotation.view(1, 1, 1, 1, -1, 1).repeat([*anchors.shape[0:3], num_anchor_size, 1, 1])
            # anchors的位置+大小+旋转方向 --> [x, y, z, num_size, num_rot, 7] --> (200,200,1,1,2,7)
            anchors = torch.cat((anchors, anchor_rotation), dim=-1)  # [x, y, z, num_size, num_rot, 7]
            # 2.5 调整anchor的维度
            # 顺带说一下这里的[x,y,...]都是在7里，也就是说每一层[200,200]记录一种box的信息
            # 然后这里的2就是每个地方出两个anchor，也就是[200,200,0,7],[200,200,1,7]，大概是这样
            anchors = anchors.permute(2, 1, 0, 3, 4, 5).contiguous() #[x, y, z, dx, dy, dz, rot] (1，200，200，1，2，7)
